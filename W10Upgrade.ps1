...
.DESCRIPTION
    ...
    This script automatically downloads the Fido.ps1 script if not present, retrieves the Windows 10 22H2 ISO, mounts it, copies the setup files, and initiates the upgrade process.

.NOTES

    - The script requires internet access to download Fido.ps1 and the Windows ISO.
...
function Mount-And-ExtractISO {
    ...
    try {
        ...
        Copy-Item -Path "$mountedPath*" -Destination $ExtractPath -Recurse -Force -Verbose | Tee-Object -Variable copyOutput

        if ($LASTEXITCODE -ne 0) {

            Write-Log "Error occurred during copying setup files." "ERROR"
...
try {
    ...
    Create-DirectoryIfNotExists -Path "C:\WindowsSetup"
    ...
    $SetupExePath = "C:\WindowsSetup\setup.exe"
    ...
    # Confirm with the user before starting the upgrade

    $userInput = Read-Host "The Windows upgrade process will now begin. Your system will restart automatically after the upgrade. Do you want to continue? (Y/N)"

    if ($userInput -ne 'Y' -and $userInput -ne 'y') {

        Write-Log "Upgrade process aborted by the user."
